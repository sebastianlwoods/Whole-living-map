// shipper-monitor-version: 2026-07-21-free-preview-paywall-v1
(function () {
  "use strict";

  const MONITOR_VERSION = "2026-07-21-free-preview-paywall-v1";
  ...
  const SCAFFOLD_INTERRUPTION_TEXT_FINGERPRINTS = ["Uh oh, something interrupted the AI","preview couldn't be completed due to a temporary technical issue"];
  const SCAFFOLD_PROMPT_TEXT_FINGERPRINTS = ["Your home page is waiting for a prompt","The builder set up your project but didn't create this page yet","Send a follow-up prompt to build it"];
  ...
  // ===== Free Preview Dead-Click Paywall =====
  // Free-preview (design-only) builds ship decorative buttons/links. When the
  // parent app enables this mode, clicks on those dead elements are intercepted
  // and reported so the app can open the upgrade paywall instead of doing nothing.
  var freePreviewPaywallState = { enabled: false };

  function isWorkingInPageAnchor(anchor) {
    var href = anchor.getAttribute("href") || "";
    if (href.charAt(0) !== "#" || href === "#") return false;
    try {
      var id = decodeURIComponent(href.slice(1));
      if (document.getElementById(id)) return true;
      return document.getElementsByName(id).length > 0;
    } catch (err) {
      return false;
    }
  }

  function findDeadInteractiveTarget(target) {
    if (!(target instanceof Element)) return null;
    var el = target.closest(
      'a, button, [role="button"], input[type="submit"], input[type="button"]'
    );
    if (!el) return null;
    if (el.closest("[data-shipper-comment-marker]")) return null;
    var tag = el.tagName.toLowerCase();
    if (tag === "a") {
      var href = el.getAttribute("href");
      // Working in-page anchor nav (e.g. #pricing scroll links) stays functional.
      if (href && isWorkingInPageAnchor(el)) return null;
      if (!href || href.trim() === "" || href.trim() === "#" || /^javascript:/i.test(href.trim())) {
        return el;
      }
      // Real destination — let it navigate.
      return null;
    }
    // Buttons/role=button are decorative by design in free-preview builds.
    return el;
  }

  function reportFreePreviewInteraction(el) {
    postToParent({
      type: "FREE_PREVIEW_INTERACTION",
      data: {
        tag: el ? el.tagName.toLowerCase() : "form",
        text: el && el.textContent ? el.textContent.trim().slice(0, 80) : "",
        href: el && el.getAttribute ? el.getAttribute("href") : null,
      },
    });
  }

  function handleFreePreviewClick(event) {
    if (!freePreviewPaywallState.enabled) return;
    // Comment mode and the visual editor own clicks while active.
    if (commentBridgeState.enabled) return;
    if (typeof visualEditorState !== "undefined" && visualEditorState.enabled) return;
    var el = findDeadInteractiveTarget(event.target);
    if (!el) return;
    event.preventDefault();
    event.stopPropagation();
    if (typeof event.stopImmediatePropagation === "function") {
    ...
    reportFreePreviewInteraction(el);
  }

  function handleFreePreviewSubmit(event) {
    if (!freePreviewPaywallState.enabled) return;
    if (commentBridgeState.enabled) return;
    event.preventDefault();
    event.stopPropagation();
    reportFreePreviewInteraction(
      event.target instanceof Element ? event.target : null
    );
  }

  function setFreePreviewPaywall(enabled) {
    freePreviewPaywallState.enabled = Boolean(enabled);
  }

  function setupFreePreviewPaywall() {
    document.addEventListener("click", handleFreePreviewClick, true);
    document.addEventListener("submit", handleFreePreviewSubmit, true);
  }
  ...
  // A generated TanStack/React app catches render errors and paints this
  // fallback instead of letting window.onerror observe the exception. Detect
  // the specific developer-overlay signature so ordinary app copy cannot match.
  function readErrorBoundaryOverlay() {
    const root = document.querySelector(
      '#root, [id*="root"], [class*="root"], body > div:first-child'
    );
    if (!root || (root.textContent || "").indexOf("Something went wrong") === -1) {
      return null;
    }

    const hasHideError = Array.prototype.some.call(
      root.querySelectorAll("button"),
      (button) => /hide error/i.test(button.textContent || "")
    );
    const detail = root.querySelector("pre, code");
    if (!hasHideError || !detail) return null;

    return {
      message: (detail.textContent || "Something went wrong").trim(),
    };
  }

  function readScaffoldPlaceholder() {
    const root = document.querySelector(
      '#root, [id*="root"], [class*="root"], body > div:first-child'
    );
    if (!root) return null;
    ...
    for (let i = 0; i < SCAFFOLD_INTERRUPTION_TEXT_FINGERPRINTS.length; i++) {
      const fingerprint = SCAFFOLD_INTERRUPTION_TEXT_FINGERPRINTS[i];
      if (text.indexOf(fingerprint.toLowerCase()) !== -1) {
        return { message: fingerprint };
      }
    }

    const promptMatches = [];
    for (let i = 0; i < SCAFFOLD_PROMPT_TEXT_FINGERPRINTS.length; i++) {
      const fingerprint = SCAFFOLD_PROMPT_TEXT_FINGERPRINTS[i];
      if (text.indexOf(fingerprint.toLowerCase()) !== -1) {
        promptMatches.push(fingerprint);
      }
    }
    if (promptMatches.length >= 2) {
      return { message: promptMatches[0] };
    }
    return null;
  }

  let lastReportedOverlayMessage = null;
  let lastOverlayReportAt = 0;
  let overlayFirstSeenAt = 0;
  let overlaySeenMessage = null;
  function checkForErrorOverlay() {
    try {
      const overlay = readErrorBoundaryOverlay();
      if (!overlay) {
        lastReportedOverlayMessage = null;
        lastOverlayReportAt = 0;
        overlayFirstSeenAt = 0;
        overlaySeenMessage = null;
        return;
      }

      const message = overlay.message;
      const now = new Date().getTime();
      if (message !== overlaySeenMessage) {
        overlaySeenMessage = message;
        overlayFirstSeenAt = now;
      }

      // HMR can flash an error boundary for a frame. Only the same overlay
      // persisting for three seconds becomes a user-facing fault.
      if (now - overlayFirstSeenAt < 3000) return;

      // The builder can begin listening after the crash. Re-emit a persistent
      // overlay on a bounded heartbeat so the recoverable screen cannot miss it.
      if (
        message === lastReportedOverlayMessage &&
        now - lastOverlayReportAt < 4000
      ) {
        return;
      }
      lastReportedOverlayMessage = message;
      lastOverlayReportAt = now;
      postToParent({
        type: "RUNTIME_ERROR",
        data: {
          message,
          blankScreen: false,
          source: "error-boundary-overlay",
        },
      });
    } catch (err) {
      // Detection must never throw into the generated app.
    }
  }

  let lastReportedScaffoldMessage = null;
  let lastScaffoldReportAt = 0;
  let scaffoldFirstSeenAt = 0;
  let scaffoldSeenMessage = null;
  function checkForScaffoldPlaceholder() {
    try {
      const scaffold = readScaffoldPlaceholder();
      if (!scaffold) {
        lastReportedScaffoldMessage = null;
        lastScaffoldReportAt = 0;
        scaffoldFirstSeenAt = 0;
        scaffoldSeenMessage = null;
        return;
      }

      const message = scaffold.message;
      const now = new Date().getTime();
      if (message !== scaffoldSeenMessage) {
        scaffoldSeenMessage = message;
        scaffoldFirstSeenAt = now;
      }

      // A starter page can flash while HMR replaces the route. Only a scaffold
      // that remains for three seconds is evidence the generation never landed.
      if (now - scaffoldFirstSeenAt < 3000) return;

      if (
        message === lastReportedScaffoldMessage &&
        now - lastScaffoldReportAt < 4000
      ) {
        return;
      }
      lastReportedScaffoldMessage = message;
      lastScaffoldReportAt = now;
      postToParent({
        type: "RUNTIME_ERROR",
        data: {
          message,
  ...
  function checkContentLoaded() {
    ...
    const errorBoundaryOverlay = Boolean(readErrorBoundaryOverlay());
    const scaffoldPlaceholder = Boolean(readScaffoldPlaceholder());
  ...
  function setupContentDetection() {
    ...
    if (
      !initialState.hasContent &&
      (initialState.errorBoundaryOverlay || initialState.scaffoldPlaceholder)
    ) {
      postToParent({
    ...
    // The fallback is static after it renders, and the parent may attach late.
    // Keep checking lightly after the initial content polling window ends.
    setInterval(checkForErrorOverlay, 2000);
    setInterval(checkForScaffoldPlaceholder, 2000);
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    if (
      ...
      type === "ENABLE_FREE_PREVIEW_PAYWALL" ||
      type === "DISABLE_FREE_PREVIEW_PAYWALL"
    ) {
      console.log(
      ...
      } else if (type === "ENABLE_FREE_PREVIEW_PAYWALL") {
        setFreePreviewPaywall(true);
      } else if (type === "DISABLE_FREE_PREVIEW_PAYWALL") {
        setFreePreviewPaywall(false);
  ...
  // ===== Build / Compile Error Tracking (Vite HMR) =====
  // Build errors (syntax errors, unresolved imports, missing modules) never fire
  // window.onerror — the dev server keeps running and Vite renders its own
  // <vite-error-overlay> instead. We observe that overlay and forward the error
  // Vite already parsed (message/file/frame), so the editor no longer has to
  // regex-scrape raw vite.log text server-side to know the preview is broken.
  function setupViteBuildErrorTracking() {
    var OVERLAY_TAG = "vite-error-overlay";
    var lastKey = null;

    function readOverlay(overlay) {
      try {
        var root = overlay.shadowRoot || overlay.root;
        if (!root) return null;
        var text = function (sel) {
          var el = root.querySelector(sel);
          return el ? (el.textContent || "").trim() : "";
        };
        // Class names rendered by Vite's ErrorOverlay shadow DOM.
        var message = text(".message-body") || text(".message");
        if (!message) return null;
        return {
          message: message.slice(0, CONFIG.MAX_STRING_LENGTH),
          file: text(".file") || undefined,
          frame: text(".frame").slice(0, CONFIG.MAX_STRING_LENGTH) || undefined,
          stack: text(".stack").slice(0, CONFIG.MAX_STRING_LENGTH) || undefined,
          plugin: text(".plugin") || undefined,
        };
      } catch (e) {
        return null;
      }
    ...
    function emit(overlay) {
      var err = readOverlay(overlay);
      if (!err) return;
      var key = err.message + "|" + (err.file || "");
    ...
    // An error overlay may already be present if the build broke before the
    // monitor script finished loading.
    var existing = document.querySelector(OVERLAY_TAG);
    ...
    var observer = new MutationObserver(function (mutations) {
      ...
      for (var i = 0; i < mutations.length; i++) {
        ...
        for (var a = 0; a < m.addedNodes.length; a++) {
          if (isOverlayNode(m.addedNodes[a])) emit(m.addedNodes[a]);
        }
        for (var r = 0; r < m.removedNodes.length; r++) {
          if (isOverlayNode(m.removedNodes[r])) removed = true;
        }
      }
      // Overlay removed and none left => Vite cleared the error (HMR recovered).
      if (removed && !document.querySelector(OVERLAY_TAG)) {
        lastKey = null;
        postToParent({ type: "BUILD_ERROR_CLEARED", data: {} });
      }
    });
    observer.observe(document.documentElement, {
      childList: true,
  ...
  function init() {
    ...
    // A real (full-document) navigation blanks this document; tell the parent
    // so it can cover with its loader instead of exposing the white unload gap.
